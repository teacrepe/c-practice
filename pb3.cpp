...
void sortBook(Sperson person[], int i)
{
	qsort(person, i, sizeof(Sperson), compare);
}

void saveBook(Sperson person[], int i)
{
	FILE* fp;
	fp = fopen("pblist", "wb");
	...
	{
		printf("Saved %s\n", person[j].name);
		fwrite(&(person[j]), sizeof(Sperson), 1, fp);
	}
	fclose(fp);
}

void loadBook(Sperson person[], int* personCount)
{
	FILE* fp;
	Sperson sp, *spp = &person[0];
	fp = fopen("pblist", "rb");
	int i = 0;
	while (fread(spp, sizeof(Sperson), 1, fp) > 0)
	{
		printf("Loaded %s\n", spp->name);
		spp = &(person[++i]);
		(*personCount)++;
